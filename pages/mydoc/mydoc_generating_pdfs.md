...
echo 'Kill all Writarium instances'
...
After stopping all Writarium instances, we build Writarium using a special configuration file that specifies a unique stylesheet. The build contains a file (prince-list.txt) that contains a list of all pages to be included in the PDF. We feed this list into a Prince command to build the PDF.
...
output: pdf
product: mydoc
print_title: Writarium theme for documentation — mydoc product
...
```
echo 'Killing all Writarium instances'
kill -9 $(ps aux | grep '[j]ekyll' | awk '{print $2}')
clear
...
Note that the first part kills all Writarium instances. This way you won't try to serve Writarium at a port that is already occupied.
